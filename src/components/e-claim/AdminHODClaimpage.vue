  ...
  >
    <div class="container mx-auto text-xs lg:text-base">
      ...
      >
        ...
        >
          <div
            class="bg-white w-full sm:w-4/5 rounded-xl flex flex-col items-center relative pb-6 dark:bg-gray-800"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
    ...
    async ApproveOrReject(AoR) {
      ...
      this.singleRemarks.forEach((remark) => {
        ...
        if (remark.Tab_Title == 'Local Outstation') {
          axios.put(
            'http://172.28.28.117:7165/api/Admin/Approver_Comment_Local',
            data
          );
        } else if (remark.Tab_Title == 'Overseas Outstation') {
          axios.put(
            'http://172.28.28.117:7165/api/Admin/Approve_Comment_Overseas',
            data
          );
        } else if (remark.Tab_Title == 'Staff Refreshment') {
          axios.put(
            'http://172.28.28.117:7165/api/Admin/Approve_Comment_Refreshment',
            data
          );
        } else if (remark.Tab_Title == 'Entertainment') {
          axios.put(
            'http://172.28.28.117:7165/api/Admin/Approve_Comment_Entertainment',
            data
          );
        } else if (remark.Tab_Title == 'Other') {
          axios.put('http://172.28.28.117:7239/api/Verifier/VerifierOthers', data);
        }
      });
      ...
      if (AoR == 'Approve') {
        ...
        const approveData = {
          status: 'APPROVED BY APPROVER. WAITING FOR FINANCE.',
        ...
        await axios
          .put(
            'http://172.28.28.117:7165/api/Admin/Approve_Claim_HOD',
            approveData
          )
          .then((response) => {
        ...
        const approveData = {
          status: 'RESUBMIT REQUESTED BY APPROVER.',
          comment: this.remark ? this.remark : '',
          requester_email: 'user_email',
          verifier_email: this.claimDetails.verifier_email,
          reference_number: this.claimDetails.reference_number,
