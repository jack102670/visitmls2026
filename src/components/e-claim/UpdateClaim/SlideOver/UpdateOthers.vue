      ...
      <form @submit.prevent="handleSubmit">
        <div class="grid grid-cols-8 gap-2 w-full">
          <div class="col-span-4">
            <label for="expense_date" class="font-medium text-sm"
              >Expense Date</label
            >
            <input
              type="date"
          ...
          <div class="col-span-4">
            <label for="expense_name" class="font-medium text-sm"
              >Expense Name</label
            >
            <input
              type="text"
              id="expense_name"
              v-model="others.expense_name"
              class="mt-1 text-xs block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
            />
          </div>
          ...
          <div class="col-span-4">
            <label for="total_fee" class="font-medium text-sm"
              >Total Amount(RM)</label
            >
            <input
              type="text"
              id="total_fee"
              v-model="others.total_fee"
              class="mt-1 text-xs block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
            />
          </div>
          <div class="col-span-4">
            ...
            <div v-if="others.files.length" class="mt-2">
              <!-- <p class="text-xs text-gray-600">
                Click on a file to view or delete:
              </p> -->
              <ul class="list-disc list-inside">
                <li
                  v-for="(file, index) in others.files"
                  :key="index"
  ...
  methods: {
    async fetchOthersData(refNo) {
      const response = await axios.get(
        `http://172.28.28.116:6239/api/User/GetOthers/${refNo}`
      );
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          if (
            ...
            !this.newFiles.find((f) => f.name === deletedFile)
          ) {
    ...
    async handleSubmit() {
      ...
      console.log("Submitting Others payload:", updatedClaim);
