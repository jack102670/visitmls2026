    ...
    <div v-if="isOpen" class="fixed inset-y-0 right-0 w-1/3 bg-white dark:bg-gray-800 z-50 p-6 overflow-y-auto">
      ...
      <form @submit.prevent="handleSubmit">
        <div class="grid grid-cols-8 gap-2 w-full">
          ...
          <div class="col-span-4">
            <label for="description" class="font-medium text-sm">Purpose</label>
          ...
          <div class="col-span-4">
            ...
            <div class="flex space-x-2">
              <div class="mt-1" v-if="overseas.transport_mode === 'Personal Transport'">
                <label class="inline-flex items-center">
                  <input type="radio" v-model="overseas.transport_mode" value="Personal Transport"
                    class="form-radio text-indigo-600 text-xs">
                  <span class="ml-2 text-xs">Personal Transport</span>
                </label>
              </div>
              <div class="mt-1" v-if="overseas.transport_mode === 'Company Transport'">
                <label class="inline-flex items-center">
                  <input type="radio" v-model="overseas.transport_mode" value="Company Transport"
                    class="form-radio text-indigo-600 text-xs">
                  <span class="ml-2 text-xs">Company Transport</span>
                </label>
              </div>
              <div class="mt-1" v-if="overseas.transport_mode === 'Public Transport'">
                <label class="inline-flex items-center">
                  <input type="radio" v-model="overseas.transport_mode" value="Public Transport"
                    class="form-radio text-indigo-600 text-xs">
          ...
          <div class="col-span-4">
            ...
            <select v-if="overseas.transport_mode === 'Public Transport'" id="transport_specification"
          ...
          <div class="col-span-4" v-if="overseas.transport_mode === 'Company Transport'">
            <label for="type_petrol" class="font-medium text-sm">Type of Fuel</label>
            <select id="type_petrol" v-model="overseas.type_petrol"
          ...
          <div class="col-span-4" v-if="overseas.transport_mode === 'Company Transport'">
            ...
            <input type="text" id="total_mileage" v-model="overseas.total_mileage"
              class="mt-1 text-xs block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
          </div>

          <div class="col-span-4" v-if="overseas.transport_mode === 'Company Transport'">
            <!-- Upload field shown if mileage entered and no files yet -->
            <div v-if="overseas.total_mileage && !categorizedFiles.mileage.length">
              <input type="file" id="newFile" @change="uploadFiles($event, 'mileage')"
                class="mt-1 text-xs block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
            ...
            <div v-if="overseas.total_mileage && categorizedFiles.mileage.length">
              ...
              <ul class="mt-1 text-xs text-blue-600 underline">
                <li v-for="(file, index) in categorizedFiles.mileage" :key="index" class="flex items-center gap-2">
                  ...
                  <a @click="deleteFile(index, 'mileage')"
          ...
          <div class="col-span-4"
            v-if="overseas.transport_mode === 'Public Transport' && overseas.transport_specification === 'Flight'">
          ...
          <div class="col-span-4"
            ...
            <select id="class_airline" v-model="overseas.class_airline"
              class="mt-1 text-xs block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
          ...
          <div class="col-span-4" v-if="overseas.transport_mode === 'Public Transport'">
            ...
            <div v-if="overseas.fare && !categorizedFiles.fare.length" class="mt-2">
              <input type="file" id="newFile" @change="uploadFiles($event, 'fare')"
                class="mt-1 text-xs block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
              <span v-if="selectedFileName" class="text-xs text-gray-600 mt-1 block">
                Selected file: {{ selectedFileName }}
              </span>
          ...
          <!-- <div class="grid grid-cols-8 gap-2 w-full"> -->
            <div class="col-span-4">
                ...
                <input type="text" id="hotel_fee" v-model="overseas.hotel_fee"
                    class="mt-1 text-xs block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"/>
            </div>

            <div class="col-span-4">
                <!-- Upload field shown if hotel entered and no files yet -->
                <div v-if="overseas.hotel_fee && !categorizedFiles.hotel.length" class="mt-2">
                    <input
                    type="file"
                    id="newFile"
          ...
          <div class="col-span-4" v-if="overseas.transport_mode !== 'Public Transport'">
            ...
            <div v-if="overseas.toll_fee && !categorizedFiles.toll.length" class="mt-2">
              <input type="file" id="newFile" @change="uploadFiles($event, 'toll')"
                class="mt-1 text-xs block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
              <span v-if="selectedFileName" class="text-xs text-gray-600 mt-1 block">
                Selected file: {{ selectedFileName }}
              </span>
          ...
          <div class="col-span-4" v-if="overseas.transport_mode !== 'Public Transport'">
            ...
            <div v-if="overseas.park_fee && !categorizedFiles.parking.length" class="mt-2">
              <input type="file" id="newFile" @change="uploadFiles($event, 'parking')"
                class="mt-1 text-xs block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
              <span v-if="selectedFileName" class="text-xs text-gray-600 mt-1 block">
                Selected file: {{ selectedFileName }}
              </span>
            ...
            <div v-if="overseas.park_fee && categorizedFiles.parking.length" class="mt-4">
              ...
              <ul class="mt-1 text-xs text-blue-600 underline">
                <li v-for="(file, index) in categorizedFiles.parking" :key="index" class="flex items-center gap-2">
                  ...
                  <a @click="deleteFile(index, 'parking')"
                    class="text-red-500 transition-colors duration-200 dark:hover:text-yellow-500 dark:text-gray-300 hover:text-yellow-500 focus:outline-none">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                      stroke="currentColor" class="w-5 h-5">
                      <path stroke-linecap="round" stroke-linejoin="round"
          ...
          <div class="col-span-4">
            ...
            <div v-if="overseas.files">
              ...
              <ul class="list-disc list-inside">
                <li v-for="(file, index) in categorizedFiles.other" :key="index" class="flex items-center space-x-2">
                  ...
                  <a @click="deleteFile(index, 'other')"
                    class="text-red-500 transition-colors duration-200 dark:hover:text-yellow-500 dark:text-gray-300 hover:text-yellow-500 focus:outline-none">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                      stroke="currentColor" class="w-5 h-5">
                      <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div v-if="overseas.files" class="mt-4">
              <input type="file" id="newFile" @change="uploadFiles($event, 'other')"
                class="mt-1 text-xs block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
              <!-- <span v-if="selectedFileName" class="text-xs text-gray-600 mt-1 block">
                            Selected file: {{ selectedFileName }}
                        </span> -->
          ...
          <div class="col-span-8">
            ...
            <div class="overflow-x-auto border border-gray-300 rounded-md mt-2">
              <table class="min-w-full bg-white dark:bg-gray-800 border-collapse">
                ...
                <tbody>
                  <tr v-for="(expenses, index) in paginatedExpenses || []" :key="expenses.id"
          ...
          <div class="col-span-8 justify-end items-center flex space-x-2 py-4">
            ...
            <button type="submit" class="w-full bg-blue-900 text-white hover:bg-blue-900 font-bold py-2 px-4 rounded">
              Update
            </button>
          </div>
        </div>
      ...
      <transition name="fade">
        <div v-if="isExpensesFormOpen" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div class="bg-white p-6 rounded-lg shadow-lg w-1/3">
            ...
            <div>
              ...
              <input type="text" v-model="selectedExpenses.exchange_rate" @input="updateAmountsInRM(selectedExpenses)"
                class="w-full border rounded p-2" :readonly="isForeignFieldsReadOnly" />
            </div>

            <div>
              <label class="block text-sm font-medium">Amount(Foreign Currency)</label>
              <input type="text" v-model="selectedExpenses.currency_total" @input="updateAmountsInRM(selectedExpenses)"
                class="w-full border rounded p-2" :readonly="isForeignFieldsReadOnly" />
            </div>
            ...
            <div class="flex justify-end mt-4 space-x-2">
              <button @click="isExpensesFormOpen = false" class="bg-red-400 text-white px-4 py-2 rounded">
                Cancel
              </button>
              <button @click="saveExpenses(selectedExpenses)" class="bg-[#160959] text-white px-4 py-2 rounded">
                Save
              </button>
            </div>
          </div>
...
export default {
    ...
    computed: {
        ...
        const mileage = parseFloat(this.overseas.mileage_km) || 0;
        ...
        checkin_date: {
            get() {
                const d = new Date(this.overseas.checkin_date);
                if (isNaN(d)) return "";
                return d.toISOString().split("T")[0];
            },
            set(value) {
            this.overseas.checkin_date = value;
            },
        },
        checkout_date: {
            get() {
                const d = new Date(this.overseas.checkout_date);
                if (isNaN(d)) return "";
                return d.toISOString().split("T")[0];
            },
            set(value) {
            this.overseas.checkout_date = value;
            },
        },
        totalPages() {
...
// },
        ...
        if (matchingUniqueID) {
            ...
            (this.overseas.files || []).forEach(file => {
        ...
        async handleSubmit() {
          const updatedClaim = {
              ...
              hotel_fee: isNaN(parseFloat(this.overseas.hotel_fee)) ? 0 : parseFloat(this.overseas.hotel_fee),
