<template>
  <transition name="overlay">
    <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 z-50 overflow-auto" @click="closeSlideOver"></div>
  </transition>
  <transition name="slide">
    <div v-if="isOpen" class="fixed inset-y-0 right-0 w-1/3 bg-white dark:bg-gray-800 z-50 p-6 overflow-y-auto overflow-x-hidden">
      ...
      <div class="grid grid-cols-8 gap-4 w-full">
        <div class="col-span-8">
          <div class="flex flex-col py-8">
            <h1 class="text-md font-semibold w-screen mb-6">MONTHLY REPORT HR</h1>
            <!-- Company -->
            ...
            <div class="relative mb-4">
              ...
              <div class="flex flex-wrap gap-2 mt-2">
                <span
                  v-for="(dept, index) in chosenDepartments"
                  :key="dept"
                  class="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs flex items-center"
                >
                  {{ dept }}
                  <button @click="chosenDepartments.splice(index, 1)" class="ml-1 text-blue-500 hover:text-red-600">
                    &times;
                  </button>
                </span>
              </div>
...
export default {
  ...
  methods: {
    ...
    //   // Store export data in parent component
    //   this.$emit("fetch-report", payload);


    //   // Wait for the data to load, then export to PDF
    //   setTimeout(async () => {
    //     const reportRef = this.$root.$refs.monthlyReportRef;
    ...
    //       }, 1000); // Ensure content has rendered
    //     }
