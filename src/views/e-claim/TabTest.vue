  ...
  >
    ...
    >
      ...
      >
        ...
        <!-- Integration of sub-tabs only for the Entertainment Tab -->
        <div v-if="index === entertainmentTabIndex">
          <button
            v-for="(subTab, subTabIndex) in entertainmentSubTabs"
            :key="subTabIndex"
            @click="activeSubTab = subTabIndex"
            :class="{
              'bg-gray-300': activeSubTab === subTabIndex,
              'hover:bg-gray-200': activeSubTab !== subTabIndex,
            }"
            class="px-4 py-2 mr-2 rounded-sm focus:outline-none border border-gray-300"
          >
            {{ subTab.title }}
