...
<template>
    ...
    <tr v-for="(item, index) in items" :key="index">
      ...
      <td class="px-12 py-4 text-sm font-medium text-gray-700 whitespace-nowrap">
        <span :class="`inline-flex items-center px-3 py-1 rounded-full gap-x-2 ${item.status === 'PENDING' ? 'bg-red-100/60 dark:bg-gray-800' : 'bg-green-100/60 dark:bg-gray-800'}`">
          <span :class="`h-1.5 w-1.5 rounded-full ${item.status === 'PENDING' ? 'bg-red-500' : 'bg-green-500'}`"></span>
          <span :class="`text-sm font-normal ${item.status === 'PENDING' ? 'text-red-500' : 'text-green-500'}`">{{ item.status }}</span>
        </span>
      </td>
