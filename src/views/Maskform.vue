...
export default {
  ...
  methods: {
    ...
    submitForm() {
      console.log("Submitting form data:", this.formData); // Log formData to console for debugging
      axios
        .post("http://localhost:3000/requesters", this.formData)
        .then((response) => {
          // Handle the response
          console.log("Response received:", response); // Log response for debugging
          this.requesters.push(response.data);
          this.formData = {
            requesterName: "", // Corresponds to "Requester Name"
        ...
        .catch((error) => {
          console.error("Error adding new requester:", error); // Log error for debugging
        });
    },
  },
};
