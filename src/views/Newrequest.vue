<template>
  <main class="flex-1 overflow-x-hidden text overflow-y-auto bg-[#CED1DA] dark:bg-[#111827] p-4 sm:ml-64">
    <div class="container mx-auto">
      <div
        class="bg-[#f7fbff] dark:bg-gray-800 dark:ring-offset-gray-900 border-gray-200 dark:border-gray-700 rounded-lg px-6 py-8 ring-1 ring-slate-900/5 shadow-xl">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-xl font-bold text-slate-900 dark:text-white">Dashboard</h2>

        </div>
        <hr class=" h- mx-auto   bg-gray-100 border-0 rounded  dark:bg-gray-700">
        <div class="pt-0 container px-6 py-1 mx-auto ">
          <div class="grid grid-cols-1 gap-8 mt-8 xl:mt-12 xl:gap-12 md:grid-cols-2 xl:grid-cols-3">
            <div
              class="p-6 space-y-3  h-20 border-4 bg-gray-200 dark:bg-gray-900 dark:border-gray-700 rounded-xl relative">


              <h1 class="text-xl font-semibold text-gray-700 capitalize dark:text-slate-300">On-Going Request</h1>



              <a href="#"
                class="inline-flex p-6 absolute -top-3 -right-6 text-blue-100 capitalize transition-colors duration-300 transform bg-blue-100 rounded-full rtl:-scale-x-100 dark:bg-blue-500 dark:text-blue-500 hover:underline  dark:hover:text-blue-500">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24"
                  stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M13 9l3 3m0 0l-3 3m3-3H8m13 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span class="text-blue-700 dark:text-white mx-1 top-5 font-semibold text-2xl absolute">3</span>
              </a>
            </div>


            <buttom @click="showContent"
              class="p-6 space-y-3  h-20 border-4 bg-tranparent dark:border-gray-700 dark:bg-gray-900 rounded-xl relative">


              <h1 class="text-xl font-semibold text-gray-700 capitalize dark:text-slate-300">New Request</h1>





              <a href=""
                class="inline-flex p-6 absolute -top-3 -right-6 text-blue-500 capitalize transition-colors duration-300 transform bg-blue-100 rounded-full rtl:-scale-x-100 dark:bg-blue-500 dark:text-white hover:underline hover:text-blue-600 dark:hover:text-blue-500">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24"
                  stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M13 9l3 3m0 0l-3 3m3-3H8m13 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </a>
            </buttom>
          </div>



        </div>
        <div class="container mx-auto">
          <div v-if="showAdditionalContent"
            class="mt-1 p-6 dark:bg-gray-800 rounded-lg grid grid-cols-1 xl:mt-10 xl:gap-5 md:grid-cols-2 xl:grid-cols-3 gap-3">
            <!-- Dropdown Trigger -->
            <div
              class="flex justify-center p-3 text-6xl dark:bg-gray-700 bg-gray-100 border-2 border-gray-200 rounded-xl">
              <label for="location" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Location:</label>
              <select v-model="selectedLocation" @change="onLocationChange" id="location"
                class="mt-1 block w-full py-2 px-3 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-700 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                <option v-for="location in locations" :key="location.id" :value="location.id">{{ location.name }}
                </option>
              </select>
            </div>

            <!-- Dropdown Form -->
            <div
              class="flex justify-center p-3 text-6xl dark:bg-gray-700 bg-gray-100 border-2 border-gray-200 rounded-xl">
              <label for="Department"
                class="block text-sm font-medium text-gray-700 dark:text-gray-300">Department:</label>
              <select v-model="selectedDepartment" @change="onDepartmentChange" id="Department"
                class="mt-1 block w-full py-2 px-3 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-700 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                <option v-for="Department in Departments" :key="Department.id" :value="Department.id">{{ Department.name
                }}</option>
              </select>
            </div>

            <div
              class="flex justify-center p-3 text-6xl dark:bg-gray-700 bg-gray-100 border-2 border-gray-200 rounded-xl">
              <label for="TypeOfRequest" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Type Of
                Request:</label>
              <select v-model="selectedTypeOfRequest" @change="onTypeOfRequestChange" id="TypeOfRequest"
                class="mt-1 block w-full py-2 px-3 border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-700 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                <option v-for="TypeOfRequest in TypeOfRequests" :key="TypeOfRequest.id" :value="TypeOfRequest.id">{{
                  TypeOfRequest.name }}
                </option>
              </select>
            </div>
          </div>
        </div>


        <!-- Display a list of movies from the TMDb API -->

      </div>

    </div>




  </main>
  <router-view></router-view>
</template>

<script>
export default {
  name: 'NewrequestViews',
  data() {
    return {
      showAdditionalContent: false,

      selectedLocation: null,
      locations: [
        { id: '1', name: 'HQ' },
        { id: '2', name: 'OAH' },
        { id: '3', name: 'KULIM HUB' },
        // Add more locations as needed
      ],
      Departments: [
        { id: 'Security', name: 'Safety' },
        { id: 'Safety', name: 'Safety' },
        { id: 'Maintenance', name: 'Maintenance' },
        // Add more countries as needed
      ],
      selectedDepartment: null,
      selectedTypeOfRequest: null,
      TypeOfRequests: [],
    };
    // Your other data properties
  },
  methods: {
    showContent() {
      this.showAdditionalContent = !this.showAdditionalContent;
    },
    onDepartmentChange() {
      this.TypeOfRequests = []; // Reset stateCities when the country changes

      // Hard-coded data for states/cities based on the selected country
      switch (this.selectedDepartment) {
        case 'Security':
          this.TypeOfRequests = [
            { id: 'johor', name: 'Johor' },
            { id: '5', name: 'Selangor' },
            // Add more states as needed
          ];
          break;

        case 'Safety':
          this.TypeOfRequests = [
            { id: '1', name: 'Badge Request' },
            { id: '2', name: 'Incident Report' },
            { id: '3', name: 'CCTV Footage View' },
            { id: '4', name: 'Visitor/Escort' },
            { id: '5', name: 'Mask/Testkids' },
            // Add more states as needed
          ];
          break;

        case 'Maintenance':
          this.TypeOfRequests = [
            { id: '1', name: 'Badge Request' },
            { id: '2', name: 'Incident Report' },
            { id: '3', name: 'CCTV Footage View' },
            { id: '4', name: 'Visitor/Escort' },
            { id: '5', name: 'Mask/Testkids' },
            // Add more states as needed
          ];
          break;

        // Add more cases for additional countries

        default:
          break;
      }
    },

    onTypeOfRequestChange() {
      // Redirect to different child routes based on the selected type of request
      switch (this.selectedTypeOfRequest) {
        case '1':
          // Redirect to the default child route
          this.$router.push({ name: 'NewrequestDefault' });
          break;
        case '5':
          // Redirect to the 'movie' child route
          this.$router.push({ name: 'movie' });
          break;
        // Add more cases for additional types of requests

        default:
          break;
      }
    }
    // Your other methods
  },
}
</script>
